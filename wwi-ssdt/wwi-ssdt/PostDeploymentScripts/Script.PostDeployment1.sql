...
Once the data is loaded, you can run a script to load data. Set the start year and end year for your load.

Note it is slow, a 10 year run took 18 hours on my system.

See the bottom for an explantation of the parameters
...
DECLARE @UpdateCustomFields                  bit;
DECLARE @IsSilentMode                        bit;
DECLARE @AreDatesPrinted                     bit;

SET @AverageNumberOfCustomerOrdersPerDay = 30
SET @SaturdayPercentageOfNormalWorkDay = 25
SET @SundayPercentageOfNormalWorkDay = 0
SET @UpdateCustomFields = 1
SET @IsSilentMode = 1
